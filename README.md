...
### Region grouping
Each chunk identifies its region quite directly, so we can directly group them by region.  NBT DataVersion is an additional variable to group by, but some worlds do contain chunks generated by different versions of Minecraft, so it is not reliable for every world.

### Chunk coordinate ordering
The region file header establishes a sequence of (x,z) chunks within the region file, as well as its sector length.  An ordering of three chunks would only match with another by random chance with about 1e-6 probability.  We are talking about roughly 1e4 fragments to be reassembled, so collisions between the 1e3 region file selections are likely if we consider attempted matching by two-chunk sequences, but unlikely if we restrict to four-chunk sequences.

### Seed determination from chunk
First, we need to obtain a collection of seeds, most likely through the discovery of level.dat files through carving.  Then test the collection of seeds against features in the chunk data.  Structures are the simplest candidate feature.  This is sufficient to identify seeds for all regions of reasonable size.

A second and more ambitious program would test seeds against the biomes and/or terrain of the chunk.  This could potentially group chunks into their proper saved worlds quite directly -- except for worlds which share the same seed (including backed-up worlds).  Not even the location of the block in the filesystem would be necessary, and a missing region file header would be no problem.  However, this would not work for worlds that have used different generation options, of which there are certainly some.  Furthermore, the minecraft version influences both biome and terrain output to some extent, so this strategy would require quite a bit more work for what may be limited returns.  So let's leave this method aside for now.
